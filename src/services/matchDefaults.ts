...
export function loadMatchDefaults():MatchDefaults {
  ...
  try {
    ...
    const value=saved||{colors:[old?.competitorA?.color||'blue',old?.competitorB?.color||'white'],duration:old?.overtimeAttacker?300000:old?.initialDuration};
...
export function saveMatchDefaults(match:MatchState) {
  ...
  const defaults:MatchDefaults={colors:[match.competitorA.color||'blue',match.competitorB.color||'white'],duration:match.overtimeAttacker?loadMatchDefaults().duration:match.initialDuration};
