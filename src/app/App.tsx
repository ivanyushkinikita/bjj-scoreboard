...
import { canConfigureRules, useMatchStore } from '../stores/matchStore';
...
export default function App() {
  ...
  return <div className={`app ${isDisplay ? 'display' : ''}`}>
    ...
    {s.status === 'setup' && !isDisplay ? <MatchSetup/> : <main className="match">
      ...
      <div className={`timer-block ${s.status === 'finished' && !s.confirmed ? 'expired' : ''}`}><div className="status"><span className={s.status === 'running' ? 'live-dot' : 'status-dot'}/>{t(s.status === 'finished' ? s.confirmed ? 'MATCH COMPLETE' : 'TIME EXPIRED' : s.status === 'setup' ? 'WAITING FOR MATCH' : s.status.toUpperCase())}</div>{isDisplay ? <div className="timer">{formatTime(ms)}</div> : <button className="timer" aria-label={t("Edit remaining time")} disabled={s.confirmed} onClick={() => { setValue(formatTime(ms)); setDialog('time'); }}>{formatTime(ms)}</button>}<div className="timer-caption">{t(s.status === 'finished' ? s.confirmed ? 'FINAL RESULT' : 'REVIEW & CONFIRM RESULT' : isDisplay ? '\u00a0' : 'MATCH CLOCK · CLICK TO ADJUST')}</div></div>
