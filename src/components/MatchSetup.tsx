...
export function MatchSetup() {
  ...
  return <main className="setup centered-setup"><form className="setup-card" onSubmit={e=>{e.preventDefault();if(duration)useMatchStore.getState().setup(a,b,duration,rules,colors);}}>
    ...
    <div className="duration-row"><label>{t('Match duration')}<div className="duration-input"><input aria-label={t('Match duration')} className="time-input" value={time} onChange={e=>setTime(e.target.value)} placeholder={t('MM:SS')} aria-invalid={!duration}/><div className="duration-arrows"><button type="button" aria-label={t('Increase duration by 1 minute')} title={t('Increase duration by 1 minute')} disabled={!duration||duration>=59999000} onClick={()=>duration&&setTime(formatTime(Math.min(59999000,duration+60000)))}>▲</button><button type="button" aria-label={t('Decrease duration by 1 minute')} title={t('Decrease duration by 1 minute')} disabled={!duration||duration<=1000} onClick={()=>duration&&setTime(formatTime(Math.max(1000,duration-60000)))}>▼</button></div></div></label><div><small>{t('Preset duration')}: {ruleMinutes(rules)}{t('m')}{duration!==ruleMinutes(rules)*60000?` · ${t('Manual time')}`:''}</small><div className="presets">{[2,3,4,5,6,7,8,10].map(n=><button className={duration===n*60000?'selected':''} type="button" key={n} onClick={()=>setTime(formatTime(n*60000))}>{n}{t('m')}</button>)}</div></div></div>
