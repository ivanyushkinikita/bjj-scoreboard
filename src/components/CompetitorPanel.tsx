...
export function CompetitorPanel({ side, competitor: c, display, disabled, edit }: { side: Side; competitor: Competitor; display: boolean; disabled: boolean; edit: () => void }) {
  ...
  return <section className={`competitor ${color}`} aria-label={t('Competitor {side} scoreboard', {side})}>
    ...
    <h2 title={c.name}>{c.name || t(`Competitor ${side}`)}</h2><div className="score" data-testid={`score-${side}`}>
      {!display && <button className="score-adjustment score-decrement" aria-label={t(`${side} {field} minus`, {field: locale === 'ru' ? t('points') : 'points'})} disabled={disabled || !!match.overtimeAttacker || c.points === 0} onClick={() => score(side, 'points', -1, 'Points')}></button>}
      <span className="score-value" style={{fontSize:`min(90cqh, ${140 / String(c.points).length}cqw)`}}>{c.points}</span>
      {!display && <button className="score-adjustment score-increment" aria-label={t(`${side} {field} plus`, {field: locale === 'ru' ? t('points') : 'points'})} disabled={disabled || !!match.overtimeAttacker} onClick={() => score(side, 'points', 1, 'Points')}></button>}
    </div>
