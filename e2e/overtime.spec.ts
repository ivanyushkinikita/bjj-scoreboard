...
test('tied expiry configures a separate extra clock, syncs display and survives reload',async({page,context})=>{
  ...
  await expect(page.locator('.timer')).toHaveText('01:30');await expect(display.locator('.timer')).toHaveText('01:30');await expect(display.getByTestId('regulation-clock')).toContainText('05:00');await expect(display.getByTestId('regulation-clock').locator('strong')).toHaveText('00:00');await expect(display.getByTestId('score-A')).toHaveText('2');
  for(const surface of [page,display]){
    for(const [width,height] of surface===page?[[1000,680],[1366,768],[1920,1080]]:[[800,500],[1280,720],[1920,1080]]){
      await surface.setViewportSize({width,height});const frozen=await surface.getByTestId('regulation-clock').boundingBox(),timer=await surface.locator('.timer').evaluate(el=>{const range=document.createRange();range.selectNodeContents(el);return {x:range.getBoundingClientRect().x};});expect(frozen!.x+frozen!.width).toBeLessThan(timer!.x);await expect(surface.getByTestId('regulation-clock')).toBeInViewport();
    }
  }
  ...
  await page.keyboard.press('Space');await expect(page.locator('.timer')).not.toHaveText('01:30');await page.keyboard.press('Space');const paused=await page.locator('.timer').innerText();await expect(display.locator('.timer')).toHaveText(paused);await expect(page.getByTestId('regulation-clock').locator('strong')).toHaveText('00:00');
  await page.reload();await page.getByRole('button',{name:'ВОССТАНОВИТЬ СХВАТКУ'}).click();await expect(page.locator('.timer')).toHaveText(paused);await expect(page.getByTestId('regulation-clock')).toContainText('05:00');
  ...
  await page.getByLabel('A: плюс 2',{exact:true}).click();await page.getByLabel('Изменить оставшееся время').click();await page.getByRole('textbox',{name:/Оставшееся время/}).fill('00:01');await page.getByRole('button',{name:'Подтвердить изменение'}).click();await page.keyboard.press('Space');await expect(page.locator('.timer')).toHaveText('00:00');await expect(modal).not.toBeVisible();await expect(page.getByRole('button',{name:'Настроить дополнительное время'})).toHaveCount(0);
